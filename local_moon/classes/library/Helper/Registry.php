...
class Registry implements \JsonSerializable, \ArrayAccess, \IteratorAggregate, \Countable
{
    ...
    public function loadString($data, $format = 'JSON', array $options = [])
    {
        ...
        $obj = $this->parseString($data, $format);
     ...
     */
    protected function parseString(string $data, string $format = 'JSON'): object
    {
    ...
    {
        return $this->formatObjectToString($format, $options);
    }

    protected function formatObjectToString(string $format = 'JSON', array $options = []): string
    {
        $format = strtoupper($format);
        $data = $this->toArray();
    ...
    protected function arrayToXml($data, \SimpleXMLElement &$xml): void
    {
    ...
    protected function escapeIniValue($value): string
    {
        if (\is_numeric($value)) {
            return (string) $value;
        }
