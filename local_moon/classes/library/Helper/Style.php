...
class Style
{
    ...
    {
        if (is_string($object) && Utilities::isJsonString($object)) {
    ...
    public static function getFontFamilyValue($value, $alt_font = ''): string
    {
        ...
        $return = [];
        $font = Font::getFontFamily($value);
        if (!empty($font)) {
            $return[] = $font;
        }
        $alt_font = Font::getFontFamily($alt_font);
        if (!empty($alt_font)) {
            $return[] = $alt_font;
        }
        return implode(', ', $return);
    ...
    public static function getGradientValue($value): string
    {
        if (empty($value)) {
            return '';
        }
